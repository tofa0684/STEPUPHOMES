...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (index === -1) return NextResponse.json({ error: 'Not found' }, { status: 404, headers: corsHeaders });
    ...
    return NextResponse.json({ error: 'Failed to update testimonial' }, { status: 500, headers: corsHeaders });
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete testimonial' }, { status: 500, headers: corsHeaders });
  }
}
